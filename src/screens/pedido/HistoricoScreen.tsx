...
const PedidoItemCard = ({ pedido, onPress }: { pedido: Pedido; onPress: () => void }) => {
  ...
  const valorTotal = formatCurrency(pedido.valorTotal);

  return (
    <TouchableOpacity
      onPress={onPress}
      className="rounded-2xl bg-white border border-pedra-mar p-4 mb-3 flex-row items-center"
      activeOpacity={0.7}
    >
      <View className="flex-1">
        <View className="flex-row justify-between items-center mb-1">
          <Text className="font-bold text-ardosia text-base">{pescadorNome}</Text>
        ...
        <Text className="text-marinha text-xs mb-3">Ref: #MANG-{pedido.id.slice(-4).toUpperCase()}</Text>

        <View className="flex-row justify-between items-center">
          <View className="flex-row items-center">
            <Feather name="calendar" size={14} color="#6B655A" />
            <Text className="text-marinha text-xs ml-1">{dataFormatada}</Text>
          </View>
          <Text className="font-bold text-ardosia">{valorTotal}</Text>
        </View>
      </View>
      <View className="ml-3">
        <Ionicons name="chevron-forward" size={20} color="#D6CFC4" />
      </View>
    </TouchableOpacity>
  );
};
...
export default function HistoricoScreen() {
  ...
  const { historico: pedidos, loading, listarHistorico } = usePedidoStore();
  ...
  const renderEmptyState = () => (
    <View className="flex-1 items-center justify-center p-8 mt-12">
      <View className="bg-terracota/10 p-6 rounded-full mb-4">
        <Feather name="shopping-bag" size={48} color="#D45D4A" />
      </View>
      <Text className="text-xl font-bold text-ardosia text-center mb-2">
        Nenhum pedido por aqui
      </Text>
      <Text className="text-marinha text-center">
    ...
    <View className="flex-1 bg-areia">
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#D45D4A']} />
          }
        />
      )}
    </View>
